...
def update_config(libdir, values = {}):
    ...
    filename = os.path.join(libdir, 'up_clock_lib/up_clockconfig.py')
    oldvalues = {}
    try:
        fin = file(filename, 'r')
...
def move_desktop_file(root, target_data, prefix):
    ...
    old_desktop_path = os.path.normpath(root + target_data +
                                        '/share/applications')
    old_desktop_file = old_desktop_path + '/up-clock.desktop'
    ...
    if not os.path.exists(old_desktop_file):
        ...
        desktop_file = desktop_path + '/extras-up-clock.desktop'
        try:
...
def update_desktop_file(filename, target_pkgdata, target_scripts):
    ...
    try:
        ...
        for line in fin:
            if 'Icon=' in line:
                line = "Icon=%s\n" % (target_pkgdata + 'media/up-clock.svg')
            elif 'Exec=' in line:
                cmd = line.split("=")[1].split(None, 1)
                line = "Exec=%s" % (target_scripts + 'up-clock')
                if len(cmd) > 1:
...
class InstallAndUpdateDataDirectory(DistUtilsExtra.auto.install_auto):
    def run(self):
        ...
        target_pkgdata = target_data + 'share/up-clock/'
        target_scripts = '/' + os.path.relpath(self.install_scripts, self.root) + '/'

        values = {'__up_clock_data_directory__': "'%s'" % (target_pkgdata),
...
=======
    version='4.3',
>>>>>>> dc000c1298a12b19e082b46cdacac44f6e9c56bd
    license='GPL-3',
